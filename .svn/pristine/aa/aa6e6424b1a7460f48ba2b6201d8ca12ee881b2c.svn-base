...
public class LinView extends LinearLayout {
	...
	@SuppressLint("NewApi")
	public LinView(Context context, AttributeSet attrs, int defStyle) {
	...
	public boolean onTouchEvent(final MotionEvent event) {
		...
		if (event.getActionMasked() == MotionEvent.ACTION_DOWN
			...
			mRadiusAnimator = ObjectAnimator.ofFloat(this, "radius", dp(50),
					targetRadius);
			mRadiusAnimator.setDuration(50);
			mRadiusAnimator
					.setInterpolator(new AccelerateDecelerateInterpolator());
			mRadiusAnimator.addListener(new Animator.AnimatorListener() {
				...
				public void onAnimationEnd(Animator animator) {
					setRadius(0);
					ViewHelper.setAlpha(LinView.this, 1);
					mIsAnimating = false;
				}
	...
	protected void onDraw(final Canvas canvas) {
		...
		// 回调画布时 屏幕分辨率低于800*480不能展现画布出现报错，暂时未修复。
